...
from .metrics import InceptionScore, bn_regularizer, group_consistency, canny_consistency,  cos_consistency
...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img, a_cay, ca, gr, img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
...
#             print(self.num_images)
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels= self._run_trial( x[trial], input_data,  times, ca,  gr, labels, dryrun=dryrun)
                # Finalize
...
#                     x[trial] = x_trial
    ...
    def _run_trial(self,   x_trial, input_data, times, ca,  gr,  labels, dryrun=False):
        ...
        dm, ds = self.mean_std
        print(f'change = {max_iterations //1.5}')
...
#                                                                      max_iterations //1.142], gamma=0.1)
        ...
        writer1 = SummaryWriter("loss")
        x = torch.full([1, 3, 224, 224],0.5, **self.setup)
        try:
            for iteration in range(max_iterations):
                ca =[]
                ti = ti+1
                closure = self._gradient_closure( optimizer, x_trial, input_data, ca, gr, labels, ti, x)
                ...
                l_loss =rec_loss 
                writer.add_scalar('loss', l_loss.item(), iteration)
        ...
        print(f'min value = {min(loss)}')
        print(f'min value site = {min_site}')
        writer1.close() 
        writer.close() 
        return x_trial.detach(), labels

    def _gradient_closure(self, optimizer, x_trial, input_gradient, ca, gr,  label, ti, x):

        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
#             print(f'x_shape= {x.shape}')
#             CA= self.config['alpha_cay']
#             print(f'CA_F = {CA}')
...
#                 gradient1 = torch.tensor(gradient)
#                 input_gradient1=  torch.tensor(input_gradient)
#                 gradient1 =list(gradient)
# #                 print(f'gradient = {len(gradient1[0])}')
# #                 print(f'gradient = { gradient1[0].shape}')
#                 input_gradient1 =  list(input_gradient)
#                 print(f'input_gradient1 = {len(input_gradient1[0])}')
#                 print(f'gradient = { gradient[0].shape}')
#                 print(f'input_gradient = { input_gradient1[0].shape}')
#                 ca = torch.tensor(ca)
#                 ca_1 = self.config['site']
#                 ca_1 = torch.tensor(ca_1)
#                 ca_1 = ca_1.add(ca)
#                 ca_1 = torch.tensor(ca_1)
#                 ca_1 = ca_1//2
#                 self.config['site'] = ca_1 
...
#                 if len(ca)!= 0  and CA!= 0:    
#                     rec_loss += CA * canny_consistency(ca, ca_1)
#                 if self.config['alpha_cos'] != 0:
#                     if(ti <=(self.config['max_iterations']*0.5)):
#                         rec_loss += self.config['alpha_cos'] * cos_consistency(x_trial, torch.full(([2, 3, 224, 224]), 0.6, **self.setup))
#                 if self.config['alpha_D2'] != 0:
# #                      for j in range(len(gradient1[0])-3):
# #                         print(f'j = {j}')
# #                         print(f'gra= {gradient1[j].shape}')
# #                         print(f'int= {input_gradient1[j].shape}')
#                         gap2 = gradient1[61] - input_gradient1[61]
#                         gap3 = torch.norm(sum(gap2).mean(), p=2)
                    
#                         rec_loss += self.config['alpha_D2'] * torch.norm(sum(gap2).mean(), p=2)
# #                      print(rec_loss)
            rec_loss.backward()
