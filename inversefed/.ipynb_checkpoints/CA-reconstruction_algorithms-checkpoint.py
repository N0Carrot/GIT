...
#         return hook
    ...
    def reconstruct(self, input_data, labels, img,  img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
...
#         print(f'weights = {self.model.state_dict()["layer1.2.bn3.running_mean"]}')
        ...
        try:
            for trial in range(self.config['restarts']):
                group_fake =[]
                x_trial, labels = self._run_trial(group_fake, x[trial], input_data, labels,dryrun=dryrun)
                # Finalize
...
#                 print(f'trial={trial}')
    ...
    def _run_trial(self, group_fake, x_trial, input_data, labels, dryrun=False):
        ...
        try:
            for iteration in range(max_iterations):
                closure= self._gradient_closure(group_fake, optimizer, x_trial, input_data, labels)
...
#         print(x_trial.detach().shape)
    ...
    def _gradient_closure(self, group_fake, optimizer, x_trial, input_gradient,  label):
        
        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            #x_trial1 = x_trial.unsqueeze(0) 
...
#             print(f'ca_1 = {ca}')
            ...
            if len(self.config['canny'])!= 0  and self.config['alpha_cay']!= 0:
#                 print('GC')
                canny_reg = self.config['canny']
#                 print(f'config = {canny_reg}')
                rec_loss += self.config['alpha_cay'] * canny_consistency(ca,canny_reg)
            rec_loss.backward()
            
            canny_reg = ca
#             print(f'canny_reg = {canny_reg}')
           
            cay = self.config['canny']
#             print(f'cay = {cay}')
            
            for i in range(len(cay)):
                for j in range(len(cay[i])):
                    canny_reg[i][j] =(canny_reg[i][j]+cay[i][j])
                    
            if(len(cay)!=0):        
                for i in range(len(canny_reg)):
                    for j in range(len(canny_reg[i])):
                        canny_reg[i][j] =canny_reg[i][j] //2
#             print(f'canny_reg+ = {canny_reg}')
            
            self.config['canny'] =  canny_reg
            
#             print(f' self.canny  ={canny_reg}')
