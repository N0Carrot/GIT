...
#         return hook
    ...
    def reconstruct(self, ac,bc, cc, dc,input_data, labels, img, ca, canny_reg, img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
...
#         print(f'weights = {self.model.state_dict()["layer1.2.bn3.running_mean"]}')
        ...
        try:
            for trial in range(self.config['restarts']):
                group_fake =[]
                x_trial, labels = self._run_trial(R_ac,R_bc,R_cc,R_dc,group_fake, x[trial], input_data, ca, canny_reg, labels, dryrun=dryrun)
                # Finalize
...
#                 print(f'trial={trial}')
    ...
    def _run_trial(self, R_ac,R_bc,R_cc,R_dc, group_fake, x_trial, input_data, ca,  canny_reg, labels, dryrun=False):
        ...
        try:
            for iteration in range(max_iterations):
                closure = self._gradient_closure(R_ac,R_bc,R_cc,R_dc, group_fake, optimizer, x_trial, input_data, ca, canny_reg, labels)
...
#         print(x_trial.detach().shape)
    ...
    def _gradient_closure(self, R_ac,R_bc,R_cc,R_dc, group_fake, optimizer, x_trial, input_gradient, ca, canny_reg, label):
...
#             ca.append([row_indexes[int(2*len(row_indexes)/3)], col_indexes[2*int(len(col_indexes)/3)]])
            ...
            if self.config['total_variation'] [R_ac]!= 0:
                rec_loss += self.config['total_variation'][R_ac]* TV(x_trial)
            if self.config['alpha_BN'][R_cc]!= 0:
                rec_loss += self.config['alpha_BN'][R_cc] * bn_regularizer(
                self.bn_reg_layer_inputs, self.bn_reg_layers
            )
            if len(self.config['alpha_l2'])!= 0:
                rec_loss += self.config['alpha_l2'][R_bc]* torch.norm(x_trial, p=2)
            if len(ca)!= 0  and self.config['alpha_cay']!= 0:
            #     #print('GC')
                 rec_loss += self.config['alpha_cay'] [R_dc]* canny_consistency(ca, canny_reg)
            rec_loss.backward()
