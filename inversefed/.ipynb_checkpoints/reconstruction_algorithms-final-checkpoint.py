...
#         return hook
    ...
    def reconstruct(self, input_data, labels, img, a_cay, ca, canny_reg, img_shape, dryrun=False, eval=True, tol=None):
...
#         print(f'weights = {self.model.state_dict()["layer1.2.bn3.running_mean"]}')
        ...
        try:
            for trial in range(self.config['restarts']):
                group_fake =[]
                x_trial, labels = self._run_trial(group_fake, x[trial], input_data,  times, ca, canny_reg, labels, dryrun=dryrun)
                # Finalize
...
#                     x[trial] = x_trial
    ...
    def _init_images(self, img_shape):
        ...
        elif self.config['init'] == 'full':
            return torch.full((self.config['restarts'], self.num_images, *img_shape), 0.7, **self.setup)
        else:
            raise ValueError()

    def _run_trial(self, group_fake, x_trial, input_data, times, ca,  canny_reg, labels, dryrun=False):
        ...
        if self.config['lr_decay']:
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                             milestones=[max_iterations // 2.667, max_iterations // 2,
                                                                         max_iterations // 1.6], gamma=0.1)   # 3/8 4/8 5/8
        # self.config['lr'] = 0.2
        print(f'times={times}')
        try:
            for iteration in range(max_iterations):
                lr = self.config['lr']
#                 print(f'LR={lr}')
#                 if (times>0 and (iteration+1) % 2000==0):
#                     self.config['lr'] = self.config['lr'] * 0.01
# #                 lr = self.config['lr']
# #                 print(f'LR={lr}')    
                closure = self._gradient_closure(group_fake, optimizer, x_trial, input_data, ca, canny_reg, labels)
...
#         print(x_trial.detach().shape)
    ...
    def _gradient_closure(self, group_fake, optimizer, x_trial, input_gradient, ca, canny_reg, label):
