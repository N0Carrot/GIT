...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img, a_cay, ca, canny_reg, gr, img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
    #def reconstruct(self, input_data, labels, img_shape=(3, 32, 32),  dryrun=False, eval=True, tol=None):
        ...
        try:
            for trial in range(self.config['restarts']):
                x_trial, labels= self._run_trial( x[trial], input_data,  times, ca, canny_reg, gr, labels, dryrun=dryrun)
                # Finalize
    ...
    def _run_trial(self,   x_trial, input_data, times, ca,  canny_reg,gr,  labels, dryrun=False):
        ...
        try:
            for iteration in range(max_iterations):
                ca =[]
                closure = self._gradient_closure( optimizer, x_trial, input_data, ca, canny_reg, gr, labels)
                ...
                l = rec_loss
#                     print(l.item())
                writer.add_scalar('loss', l.item(), iteration)  # 使用tensorboard
#                 l.backward()
#                     optimizer.step()
#                     optimizer.zero_grad()
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, ca, canny_reg, gr,  label):
