...
#         return hook
    ...
    def reconstruct(self, input_data, ground_truth, labels, img, a_cay, ca, canny_reg, img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
    #def reconstruct(self, input_data, labels, img_shape=(3, 32, 32),  dryrun=False, eval=True, tol=None):
        ...
        try:
            for trial in range(self.config['restarts']):
                group_fake =[]
                x_trial, labels= self._run_trial(group_fake, ground_truth, x[trial], input_data,  times, ca, canny_reg, labels, dryrun=dryrun)
                # Finalize
    ...
    def _init_images(self, img_shape):
        if self.config['init'] == 'randn':
            ...
            return torch.full((self.config['restarts'], self.num_images, *img_shape), 0.5, **self.setup)
        else:
            raise ValueError()

    def _run_trial(self, group_fake, gound_truth, x_trial, input_data, times, ca,  canny_reg, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            if self.config['optim'] == 'adam':
                ...
                for i in range(self.config['max_iterations']):
                    l = torch.norm(x_trial - gound_truth, p=2)
#                     print(l.item())
...
#         print(x_trial.detach().shape)
        min_site = loss.index(min(loss))
        print(f'min value site = {min_site}')
