...
def _label_to_onehot(target, num_classes=81):
...
class GradientReconstructor():
    """Instantiate a reconstruction algorithm.""" """初始化一个重构"""
    def __init__(self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1):
        """Initialize with algorithm setup."""
        self.config = _validate_config(config) #读取默认的数据
        self.model = model
        #print(f'GR model = {self.model}')
        self.setup = dict(device=next(model.parameters()).device, dtype=next(model.parameters()).dtype)

        self.mean_std = mean_std
        self.num_images = num_images

        self.bn_reg_layers = self.config['bn_reg_layers']
        #print(f'self.bn_reg_layers={self.bn_reg_layers}')
        self.bn_reg_layer_inputs = [self.model.layer1[2].bn3, self.model.layer2[3].bn3, self.model.layer3[22].bn3,self.model.layer4[2].bn3]
        
#         for i, bn_layer in enumerate(self.bn_reg_layers):
#             bn_layer.register_forward_hook(self._get_hook_for_input(i))
            # print(f'self._get_hook_for_input(i)={self._get_hook_for_input(i)}')
            # print(f'bn_layer.i={bn_layer.register_forward_hook(self._get_hook_for_input(i))}')
#             self.bn_reg_layer_inputs[i]=bn_layer
            
        
        self.group_num = self.config['group_num']
        self.group_seed = list(range(group_num)) if self.config['group_seed'] is None else self.config['group_seed']

        if self.config['scoring_choice'] == 'inception':
            self.inception = InceptionScore(batch_size=1, setup=self.setup)

        self.loss_fn = torch.nn.CrossEntropyLoss(reduction='mean') #感觉可以先省去交叉熵计算，先使用softmax计算梯度，得到标签后再用交叉熵？
        #self.iDLG = True
        self.iDLG = False
...
#         return hook
    ...
    def reconstruct(self, input_data, labels, num, img, ca, canny_reg, img_shape=(3, 213, 240), dryrun=False, eval=True, tol=None):
    #def reconstruct(self, input_data, labels, img_shape=(3, 32, 32),  dryrun=False, eval=True, tol=None):
        ...
        print("start")
        num = num
#         print(f'ac,bc,cc ={ac},{bc},{cc}')
#         print(f'R_ac,R_bc,R_cc ={R_ac},{R_bc},{R_cc}')
        start_time = time.time()
        if eval:
            self.model.eval()
            #print(f'model = {self.model.layer4[0].conv1.weight[0:1]}')
#         print(f'weights = {self.model.state_dict()["layer1.2.bn3.running_mean"]}')
        ...
        try:
            for trial in range(self.config['restarts']):
                group_fake =[]
                x_trial, labels = self._run_trial(group_fake, num, x[trial], input_data, ca, canny_reg, labels, dryrun=dryrun)
                # Finalize
                #print(f'end_x_trial{trial}')
                scores[trial] = self._score_trial(x_trial, input_data, labels)
                #x[trial] = x_trial
                #print(f'x3={x[trial][0:1]}')
#                 print(f'x_trial.shape={x_trial.shape}')
#                 print(f'trial={trial}')
                if (trial<(self.config['restarts']-1)):
                    x[trial+1]= x_trial
                else:
                    x[trial] = x_trial
    ...
    def _run_trial(self, group_fake, num, x_trial, input_data, ca,  canny_reg, labels, dryrun=False):
        
        if ((x_trial.shape[0])!=1):
            x_trial=x_trial.unsqueeze(0)
        else:
            x_trial=x_trial
        ...
        try:
            for iteration in range(max_iterations):
                closure = self._gradient_closure(group_fake, optimizer, num, x_trial, input_data, ca, canny_reg, labels)
            ...
            pass
#         print(x_trial.shape)
#         print(x_trial.detach().shape)
        return x_trial.detach(), labels

    def _gradient_closure(self, group_fake, optimizer, num, x_trial, input_gradient, ca, canny_reg, label):
        
        def closure():
            optimizer.zero_grad()
            self.model.zero_grad()
            im = x_trial.mul(255).byte()
...
#             im1 = cv2.Canny(img2, int(b.data*0.7), int(b.data*0.8))
            ...
            row_index = im1[:,im1.shape[1]//2].argsort()
            col_index = im1[im1.shape[0]//2,:].argsort()

            # print(len(row_index))
            # print(len(col_index))

            row_index1 = row_index [(im1.shape[0]-100):im1.shape[0]]
            col_index1 =col_index[(im1.shape[1]-100):im1.shape[1]]
...
# #             print(index)
# #             print(img2.shape)
                # row_index1=indices//(img2.shape[0])
                # col_index1=indices%(img2.shape[1])
                # ca = [[row_index1, col_index1]]
#             print(row_indexes)
#             print(col_indexes)
            #print(row_indexes[int(len(row_indexes)/2)], col_indexes[int(len(col_indexes)/2)])
                ca.append([row_index1[int(len(row_index1)/3)], col_index1[int(len(col_index1)/3)]])
#                 ca.append([row_indexes[int(1*len(row_indexes)//3)], col_indexes[2*int(len(col_indexes)//3)]])
                ca.append([row_index1[int(1*len(row_index1)/2)], col_index1[1*int(len(col_index1)/2)]])
                # ca.append([row_indexes[int(2*len(row_indexes)/3)], col_indexes[1*int(len(col_indexes)/3)]])
                ca.append([row_index1[int(2*len(row_index1)/3)], col_index1[2*int(len(col_index1)/3)]])
            else:
            ...
            if self.config['alpha_BN']!= 0:
                rec_loss += self.config['alpha_BN'] * bn_regularizer(
