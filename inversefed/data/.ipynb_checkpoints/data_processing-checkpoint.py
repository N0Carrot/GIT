...
from .data import _build_bsds_sr, _build_bsds_dn, _build_NUSWIDE
from .loss import Classification, PSNR

def construct_dataloaders(dataset, defs, data_path='~', shuffle=True, normalize=True):
    ...
    if dataset == 'CIFAR10':
        ...
        trainset, validset = _build_NUSWIDE(path, defs.augmentations, normalize, upscale_factor=1, RGB=True)
        loss_fn = Classification()
...
def _build_imagenet(data_path, augmentations=True, normalize=True): #加载数据集
    ...
    #validset = torchvision.datasets.ImageNet(root='/opt/data/private/LC/ImageNet/val',  transform=transforms.ToTensor())
    ...
    # Organize preprocessing
    transform = transforms.Compose([
        transforms.Resize(256), #图片大小
        transforms.CenterCrop(224), #从中间剪裁224
        transforms.ToTensor(),
